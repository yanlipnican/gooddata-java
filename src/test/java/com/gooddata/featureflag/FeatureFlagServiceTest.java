...
public class FeatureFlagServiceTest {
    ...
    public void whenClientErrorResponseThenGetFeatureFlagsShouldThrow() throws Exception {
        when(restTemplate.getForObject(matches(FEATURE_FLAGS_URI_PATTERN), eq(FeatureFlags.class), eq(PROJECT_ID)))
    ...
    public void whenEmptyResponseThenGetProjectFeatureFlagsShouldThrow() throws Exception {
        when(restTemplate.getForObject(matches(PROJECT_FEATURE_FLAGS_URI_PATTERN), eq(ProjectFeatureFlags.class), eq(PROJECT_ID))).thenReturn(null);
