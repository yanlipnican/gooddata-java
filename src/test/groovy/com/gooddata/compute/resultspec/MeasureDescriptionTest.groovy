...
class MeasureDescriptionTest extends Specification {
    ...
    def "should serialize all values"() {
        ...
        that new MeasureDescription('mId', 'als', 'fmt'),
                jsonEquals(resource('compute/resultspec/measureDescriptionFull.json'))
    }

    def "should serialize required values"() {
        expect:
        that new MeasureDescription('mId'),
                jsonEquals(resource('compute/resultspec/measureDescription.json'))
    }

    def "should deserialize all values"() {
        when:
        MeasureDescription measure = readObjectFromResource('/compute/resultspec/measureDescriptionFull.json', MeasureDescription)
    ...
    def "should deserialize required values"() {
        when:
        MeasureDescription measure = readObjectFromResource('/compute/resultspec/measureDescription.json', MeasureDescription)
